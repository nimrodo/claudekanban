...
describe("buildHtml", () => {
  it("includes script and link tags for the given asset URIs, with the nonce on script tags", () => {
    ...
    expect(html).toContain(
      '<script type="module" nonce="test-nonce" src="https://webview.example/assets/index.js"></script>',
    );
    expect(html).toContain('<link rel="stylesheet" href="https://webview.example/assets/index.css">');
  });

  it("injects the base URL global pointing at the given port, in a nonce'd inline script", () => {
    const html = buildHtml({
      scriptUris: [],
      styleUris: [],
      cspSource: "https://webview.example",
  ...
  it("sets a CSP restricting connect-src to localhost and script-src to the nonce", () => {
    const html = buildHtml({
      scriptUris: [],
      styleUris: [],
      cspSource: "https://webview.example",
