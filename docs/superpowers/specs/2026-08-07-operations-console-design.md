...
- **Domain model**: Task entity dropped for v1 — Session is the sole kanban unit. Every session, main or subagent, is its own card. Subagents are Session rows linked via `parent_session_id`, carrying an `owner` label (from `subagent_type` when the hook payload exposes it, else a generic fallback) so their card shows who owns them. Approval and Intervention are merged into one `intervention` table (`origin: 'system' | 'human'`). Artifact table dropped — a `recap` text column on `session`, populated from the raw `Stop` hook payload's final message, covers the "see the agent's recap when done" requirement without a separate table or a summarization step. **Conflict found in Phase 0, resolved post-Phase-2:** subagents do not emit their own `SessionStart`/`Stop` pair — see `spike/findings.md` ("Subagent linkage"). Phase 1 shipped a fallback that synthesized subagent `Session` rows entirely from the parent's single `PostToolUse` event, so a subagent card only ever appeared already `done`/`failed` (a known, explicitly flagged UX gap — see the Phase 1 plan's subagent-synthesis decision). **This is now fixed:** `spike/findings.md` ("Subagent running-state signal") confirms live that `SubagentStart`/`SubagentStop` hook events exist, fire before/after real subagent work (not just at completion), and correlate reliably via `agent_id` with each other and with `PostToolUse`'s `tool_response.agentId`. A subagent `Session` row is now created directly from `SubagentStart` (`status: "running"`), updated to `done`/`failed` from `SubagentStop`, with `title` still merged in later from the parent's `PostToolUse.tool_input.description` by matching `agent_id`. One filtering caveat found live: `SubagentStop` also fires with `agent_type: ""` for a session's own turn-end when that session itself runs as a background job nested under an outer process — this must be filtered out (require non-empty `agent_type`, or match against a previously-seen `SubagentStart`) before treating it as a real child-subagent signal.
...
|---|---|---|
| **Session** | id, parent_session_id (nullable — set for subagents), owner (subagent_type from the hook payload, or a generic fallback for the main session / when unavailable), title (text, nullable — subagent's `tool_input.description`; for the main session, the first line of its first `UserPromptSubmit.prompt`, full length, locked in once set and never overwritten — see `CONTEXT.md`'s **Title** entry and `spike/findings.md` "UserPromptSubmit payload shape"), status, started_at, ended_at, cwd, model, recap (text, nullable), fail_reason (text, nullable), last_activity_summary (text, nullable) | One Claude Code invocation, main or subagent. `recap` is populated from the `Stop` hook payload's final message, verbatim, when status becomes `done`. `fail_reason` carries a short failure explanation, set on the `failed` transition. `last_activity_summary` is a short human-readable summary of the session's most recent hook event (e.g. "Called Bash", "Spawned Explore subagent"), recomputed on every incoming event via `src/domain/activitySummary.ts`'s shared summarizer — surfaced on the board card while `status` is `running` or `waiting`. |
...
| Transition | Trigger |
|---|---|
| queued → running | `SessionStart` hook received (main session); `SubagentStart` hook received, filtered to non-empty `agent_type` (subagent — confirmed live, see `spike/findings.md` "Subagent running-state signal"; supersedes Phase 1's synthesize-on-`PostToolUse`-completion fallback, which skipped `queued`/`running` entirely for subagents) |
...
| running → failed | main session: `Stop`/error hook with non-zero/error payload, or ingest timeout (no event for N minutes while status=running). Subagent: `SubagentStop` hook received, where the matching parent `PostToolUse.tool_response.error` (correlated by `agent_id`) is set |
| running/review → done | main session: `Stop` hook with success payload; `recap` set from its final-message field. Subagent: `SubagentStop` hook received without a corresponding error on the parent's `PostToolUse` |
...
- Board view: columns = Session status; every session (main or subagent) is a card, showing `owner` badge, elapsed time, last event summary, and — via `parent_session_id` — visual grouping under its parent. **Revised during Phase 1 live testing:** `owner` alone was not enough to tell sessions apart (it's a hardcoded `"main"` for every top-level session), so the card headline was changed to the `cwd` basename instead, with `owner` demoted to a secondary badge. Subagent cards show `title` (see Domain model), for the same reason — `owner` only names the subagent *type* (e.g. `"Explore"`), not what it was asked to do. **Revised again, main-session title resolution (2026-08-17):** the main-session card headline now prefers `session.title` (the first line of the first prompt) over the `cwd` basename, once a `UserPromptSubmit` event has landed; `cwd` basename remains the fallback for the pre-first-prompt moment and for installs without the `UserPromptSubmit` hook registered. Card-face truncation of the (untruncated-in-storage) title is handled entirely by the existing `.card-cwd` CSS class's `text-overflow: ellipsis`, not by any backend truncation logic.
...
1. Exact set of Claude Code hook events and their payload fields — must be confirmed in Phase 0, not assumed.
   **Resolved (Phase 0):** see spike/findings.md — CLI hooks emit `SessionStart`, `PostToolUse`, and `Stop` (no `PreToolUse`/`SubagentStop` observed in that initial capture); field list captured per event. (`UserPromptSubmit` was later re-spiked and confirmed live — see item 7 below.)
...
7. Is there any hook event/field that carries the **main session's** initial task/prompt, for use as a card title? (Found during Phase 1 live testing — see `spike/findings.md` "Subagent linkage".)
   **Resolved (2026-08-17 re-spike).** `UserPromptSubmit` is confirmed live on Claude Code `2.1.233` (requires v2.1.196+) — see `spike/findings.md` "UserPromptSubmit payload shape". It fires with the user's raw prompt text in a `prompt` field (the official docs describe this field as `user_input`; the live payload disagrees — `prompt` is what actually arrives, confirmed by direct capture). This supersedes the original "never observed" finding, which predated any version-pinned, targeted test for this specific event. Implementation: `deriveMainTitle` (`src/domain/mainTitle.ts`) extracts the first line, `src/ingest/ingest.ts` sets `title` once from the first `UserPromptSubmit` per session and never overwrites it. See `CONTEXT.md`'s **Title** entry for the settled terminology.
